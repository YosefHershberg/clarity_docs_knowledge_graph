---
title: Install Clarity OData Service on Linux
tags:
  - admin
  - onprem
aliases:
  - OData Service Linux
---
# Install Clarity OData Service on Linux


Use this topic to install the Clarity OData Service (OFast application) on Linux for accessing the Clarity Data Warehouse in on-premise environments.
...
|---|---|---|
| `INSTALL_DIR` | Directory where Clarity OData Service is installed | `/opt/clarity/odata` |
...
| `RLS_ENABLED` | Enable Row Level Security (RLS). Set to `true` to enforce row-level data security based on tenant configuration. If not specified, RLS is treated as disabled. | `false` |
...
## OData Tenant Configuration on Linux


Use this topic to configure Clarity tenants for use with the OData Service in an on-premise Linux environment.
...
3. After updating both files with the correct tenant values, restart the Clarity OData Service so that the new tenant information is loaded. For restart instructions, see the Notes section above.
...
3. Restart the Clarity OData Service. For restart instructions, see the Notes section above.
...
3. Update the attributes as follows:
   ...
   |---|---|
   | `externalUrl` | The Clarity OData Service URL. Ensure this URL is accessible from all Clarity tenant hosts that are required to be loaded. |
   | `internalUrl` | Optional. Generally not recommended for on-premise environments. Clarity attempts to connect using this URL when specified, or falls back to `externalUrl`. |
