# Install Clarity OData Service on Windows (On-Premise Only)
...
- H2: Notes (#notes)
- H2: OData Tenant Configuration on Windows (On-Premise Only) (#odata-tenant-configuration-on-windows-on-premise-only)
- H3: Prerequisites (#prerequisites)
- H3: Configure Tenants During Installation (#configure-tenants-during-installation)
- H3: Configure Tenants After Installation (#configure-tenants-after-installation)
- H3: Delete Tenant Configuration (#delete-tenant-configuration)
- H3: Configure Clarity for OData (#configure-clarity-for-odata)
- H3: Verify the Tenant Integration (#verify-the-tenant-integration)

Use this topic to install the Clarity OData Service on Windows for accessing the Clarity Data Warehouse in on-premise environments. This procedure follows the same sequence of steps as the engineering installation guide and includes all required commands and configuration details.
...
```
- **Oracle Instant Client**: The Oracle Instant Client `instantclient-basiclitewindows.x64-21.18.0.0.0dbru.zip` is required. If the download fails due to Oracle download restrictions, download it manually and extract it to:
...
Use the `install.properties` file to control how the Clarity OData Service is installed. Set the following properties:
- **INSTALL_DIR**: Directory where Clarity OData Service is installed. Example: `INSTALL_DIR=C:\clarity\odata`
- **CLARITY_ODATA_PORT**: Port on which the service listens. Example: `CLARITY_ODATA_PORT=8043`
...
- **RLS_ENABLED**: Enable Row Level Security (RLS). Set to `true` to enforce row-level data security based on tenant configuration. If this property is not specified, RLS is treated as disabled. Example: `RLS_ENABLED=false`
- **RLS_FULL_ACCESS_USER_GROUP**: Full access user group used when Row Level Security (RLS) is enabled. Example: `RLS_FULL_ACCESS_USER_GROUP=odata_full_access_group`
- **RLS_NON_RLS_TABLES_ACCESSIBLE**: Allow non-RLS tables to be accessible by all users when RLS is enabled. Example: `RLS_NON_RLS_TABLES_ACCESSIBLE=true`
- **RLS_FUNCTIONS_ACCESSIBLE_FOR_ALL**: Allow RLS functions to be accessible by all users. Example: `RLS_FUNCTIONS_ACCESSIBLE_FOR_ALL=true`

## Configure Tenants

Tenants can be optionally configured during the Clarity OData Service installation process. For detailed steps, see OData Tenant Configuration on Windows (On-Premise Only).
...
## Post-Installation
- **Adding or Removing Tenants**: See OData Tenant Configuration on Windows (On-Premise Only).
- **Security**: Verify SSL configuration if HTTPS is enabled.
- **Uninstall**: Navigate to `<INSTALL_DIR>` and run `uninstall.py`.

## Troubleshooting
- **Installation errors**: Review the installation log file.
...
## OData Tenant Configuration on Windows (On-Premise Only)

Use this topic to configure Clarity tenants for use with the OData Service in an on-premise Windows environment. The steps in this guide explain how to create tenant configuration files, update the required tenant secrets, integrate the OData service with Clarity PPM, and verify that the OData endpoints are available to Clarity users.
...
- Complete all steps in Install Clarity OData Service on Windows (On-Premise Only), including installation prerequisites, user setup, and service deployment.
...
3. After updating both files with the correct tenant values, restart the OData service so that the new tenant information is loaded. For restart instructions, see the Notes section of Install Clarity OData Service on Windows (On-Premise Only).
...
3. Restart the OData Service. For restart instructions, see the Notes section of Install Clarity OData Service on Windows (On-Premise Only).
...
After completing the configuration steps, verify that OData Service has successfully loaded the tenant information and that Clarity can establish an OData connection using the configured authenticator and URLs.
